...
#'  nearestsegment.R
#'
#'  $Revision: 1.13 $  $Date: 2026/04/23 02:43:50 $
#'
...
ppllengine <- function(X, Y, action="project", check=FALSE) {
  ...
  switch(action,
         ...
         distance = {
           result <- data.frame(dist=d$min.d, which=mapXY)
         },
         project = {
           ...
           #' rotate coordinate system so x axis is parallel to line segment
           xpr <- xv * co + yv * si
           #' determine whether projection is an endpoint or interior point
           ok <- is.finite(xpr)
           left <- !ok | (xpr <= 0)
           right <- ok &  (xpr >= leng)
